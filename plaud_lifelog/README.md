...
Plaud で録音するだけで、ライフログが自動生成されます。Word のダウンロードやアップロードは不要です。

```
録音する（Plaud NOTE / NotePin）
    ↓  Plaud アプリが自動で文字起こし・要約
毎朝 6:00 に GitHub Actions が自動で：
    ↓  Plaud Web から新しい録音を取得
    ↓  Claude がライフログ・タスクを生成
    ↓  JSON として保存
毎週月曜 7:00 に自動で：
    ↓  1週間分のレポートを生成
    ↓  Discord に投稿
...
export PLAUD_BEARER_TOKEN="bearer eyJ..."   # Plaud Web 自動同期用（下記参照）
```
...
2. **F12** キーで開発者ツールを開く
3. **Console** タブで以下を入力して Enter:
   ```
   localStorage.getItem("tokenstr")
   ```
4. 表示された `"bearer eyJ..."` をコピー
5. GitHub Secrets に `PLAUD_BEARER_TOKEN` として登録
